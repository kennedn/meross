...
if jq --help | grep -q -- --slurpfile; then
  JSON=$(jq -cn \
        ...
        '{header: {from: "/appliance/\($deviceID)/publish", messageId: $messageID, method: $method, namespace: $namespace, payloadVersion: 1, sign: $sign, timestamp: $ts }, payload: $payload[0]}' )
else
  JSON=$(jq -cn \
        --arg messageID "$messageID" \
        --arg deviceID "$1" \
        ...
        '{header: {from: "/appliance/\($deviceID)/publish", messageId: $messageID, method: $method, namespace: $namespace, payloadVersion: 1, sign: $sign, timestamp: $ts }, payload: $payload}' )
fi

echo "${JSON}" | mosquitto_pub -h baby8s -p 1883 -t "/appliance/$1/subscribe" -s
